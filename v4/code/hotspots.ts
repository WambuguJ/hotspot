...
export class hotspot_page{
    ...
    get_vu(x:number, y:number):Array<number>{ 
        //
    ...
    update_coords (event:MouseEvent,coods_div:Element) {
            // 
    ...
    display_coords(event:MouseEvent):void{
        // 
        // Get current mouse position
        const x= event.clientX;
        const y = event.clientY;
    ...
    update_pixel_coords (x:number, y:number):void{
        //
        ...
        if (!hotspotElement) throw new Error("element 'hotspot' not found")            
        //
